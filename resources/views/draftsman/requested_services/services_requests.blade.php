							...
							@if($survey_requests && count($survey_requests)>0)
								...
								@foreach($survey_requests as $survey_request)
									<tr>
										...
										<td><a href="{{URL('/draftsman/service_requests_detail')}}/{{$survey_request->survey_id}}/{{$survey_request->request_status}}" style="color:#2b8fca; font-weight:bold;">HSW{{$survey_request->survey_id}}</a></td>
