							...
							@if($survey_requests && count($survey_requests)>0)
								...
								@foreach($survey_requests as $survey_request)
									<tr>
										...
										<td><a href="{{url('/surveyor/requested_service_detail')}}/{{$survey_request->id}}/{{$survey_request->request_status}}" style="color:#2b8fca; font-weight:bold;">HSW{{$survey_request->survey_id}}</a></td>
