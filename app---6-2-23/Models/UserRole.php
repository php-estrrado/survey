...
class UserRole extends Authenticatable
{
    ...
    protected $fillable = ['org_id', 'usr_role_name', 'is_active','created_by','is_deleted'];
    
    static function getUserRoles()
    {    
        $userroles = UserRole::where(function ($query) { $query->where('is_deleted', '=', NULL)->orWhere('is_deleted', '=', 0);})->orderBy('id', 'DESC')->get();     
        if($userroles)
        {
            $data      =   [];
