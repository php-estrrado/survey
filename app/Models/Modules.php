...
class Modules extends Model
{
    ...
    static function getModulesByParentId($parent)
    {
        $modules                =   Modules::where('parent',$parent)->where('is_active',1)->where(function ($query) {
            $query->where('is_deleted', '=', NULL)->orWhere('is_deleted', '=', 0);})->orderBy('sort','asc')->get(); 
...
// dd($modules);
    ...
    static function visibleModules($role){ 
        ...
        static function getModuleDetails($module_id)
        {
            $module_detail = Modules::where('id',$module_id)->where('is_deleted',0)->first();
