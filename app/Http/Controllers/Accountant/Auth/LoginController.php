...
class LoginController extends Controller
{
   ...
   // protected $redirectTo = RouteServiceProvider::HOME;
    ...
    public function loginSendotpemail(Request $request)
    {
        ...
        {
            $regexist = Admin::where('email',$request->email)->where('is_active',1)->where('is_deleted',0)->where('role_id',5)->first();
    ...
    public function regVerifyotpemail(Request $request)
    {
        ...
        if ($validator->fails()) 
            ...
            {
                $exisit = Admin::where('email',$request->email)->where('is_active',1)->where('is_deleted',0)->where('role_id',5)->first();
                if($exisit)
                {
                    $exist = Admin::where('email',$request->email)->where('otp',$request->otp)->where('is_active',1)->where('is_deleted',0)->where('role_id',5)->first();
                    if($exist)
                    {
