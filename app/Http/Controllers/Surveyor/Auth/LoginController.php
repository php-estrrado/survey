...
class LoginController extends Controller
{
   ...
   // protected $redirectTo = RouteServiceProvider::HOME;
    ...
    public function admin(){  
        return view('surveyor.auth.login');
    }
    ...
    public function loginSendotpemail(Request $request)
    {
        ...
        {
            $regexist = Admin::where('email',$request->email)->where('is_active',1)->where('is_deleted',0)->where('role_id',3)->first();
       ...
       $messages =['email.required' => 'Email field is required.', 'otp.required' => 'OTP field is required.', 'otp.numeric' => 'OTP field must be a number.', 'email.email' => 'Invalid Email.'];
        ...
        if ($validator->fails()) 
        ...
        else
            {
                ...
                if($exisit)
                {
                    $exist = Admin::where('email',$request->email)->where('otp',$request->otp)->where('is_active',1)->where('is_deleted',0)->where('role_id',3)->first();
                    if($exist)
                    {
