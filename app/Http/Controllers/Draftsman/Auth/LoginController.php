...
namespace App\Http\Controllers\Draftsman\Auth;
...
class LoginController extends Controller
{
   ...
   // protected $redirectTo = RouteServiceProvider::HOME;
    ...
    public function admin(){  
        return view('draftsman.auth.login');
    }
    ...
    public function loginSendotpemail(Request $request)
    {
        ...
        {
            $regexist = Admin::where('email',$request->email)->where('is_active',1)->where('is_deleted',0)->where('role_id',4)->first();
        ...
        else
            {
                ...
                if($exisit)
                {
                    ...
                    if (Auth::guard('admin')->attempt(['email' => $request->email, 'password' => '123456']))
                    {
                        ...
                        {
                            return redirect()->intended('/draftsman/dashboard');
                        }
                        else{
